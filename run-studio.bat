...
REM R01: never start the studio on a data root that is not at head. An interrupted
REM or failed migration must fail here, not half-start the app (SQLite DDL is not
REM transactional, so a failed upgrade is recovered from the verified backup).
powershell -NoProfile -ExecutionPolicy Bypass -File "%REPO_ROOT%scripts\preflight.ps1" -VerifyDatabase -SkipFfmpeg
if errorlevel 1 exit /b %errorlevel%
