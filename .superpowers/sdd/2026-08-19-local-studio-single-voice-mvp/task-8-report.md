...
Fixed the remaining jobs SSE mismatch:

- Backend keeps semantic SSE event name `event: job`.
- Frontend `JobProgress` now subscribes with `source.addEventListener('job', ...)` instead of relying on default `onmessage`.
- The focused fake `EventSource` test now dispatches named `job` events, so `onmessage`-only handling fails.
...
```powershell
cd frontend
npm test -- --run src/features/jobs/JobProgress.test.tsx
```

Output after changing the test fake to dispatch named events before production code:

```text
FAIL  src/features/jobs/JobProgress.test.tsx > JobProgress > loads a snapshot once and keeps a stable EventSource after named job events
TestingLibraryElementError: Unable to find an element with the text: job-2.
...
```text
ok 1 e2e\single-voice.spec.ts:3:1 › fake single narrator reaches verified publication bundle (2.2s)
1 passed (11.8s)
```
...
- Backend did not change in this round; no backend rerun was required.
- Snapshot-first behavior and `?after=` reconnect URL are preserved.
- The test now models browser behavior for named SSE events, preventing the previous false positive.
