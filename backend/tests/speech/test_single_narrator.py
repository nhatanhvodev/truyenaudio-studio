...
def test_single_plan_has_exactly_one_narrator(
    ...
    assert all(20_000 <= segment.estimated_duration_ms <= 60_000 for segment in plan.segments)
    assert "bon muoi hai ki lo met" in plan.segments[0].narration_text
    assert db_session.get(TranslationSegment, fixture.translation_segment_ids[0]).target_text == (
        "Doan mot co 42 km. "
        + "Cau van ngan giu nhip ke chuyen am ap va ro rang. " * 8
    )
    assert db_session.get(Chapter, fixture.chapter_id).state == ChapterState.VOICE_CONFIGURED.value
...
def test_target_edit_only_invalidates_corresponding_audio(
    ...
    changed = workflow.regenerate_segments(fixture.chapter_id, (rendered.segment_ids[1],))

    assert changed.reused_segment_ids == (rendered.segment_ids[0], rendered.segment_ids[2])
    assert changed.rendered_segment_ids == (rendered.segment_ids[1],)
    assert tts.requested_segment_ids == list(rendered.segment_ids) + [rendered.segment_ids[1]]
...
def test_audio_approval_is_independent_and_blocks_open_major_audio_issues(
    ...
    issue = db_session.query(QaIssue).filter(QaIssue.category == QaCategory.CLIPPING.value).one()
...
class DeterministicAudioProcessor:
    async def master(self, request, output_path: Path):
        ...
        output_path.parent.mkdir(parents=True, exist_ok=True)
        payload = "\n".join(str(path) for path in request.ordered_segment_paths).encode()
        output_path.write_bytes(payload)
        return MasterResult(
            duration_ms=90_000,
            sha256=hashlib.sha256(payload).hexdigest(),
...
class _Fixture:
    def __init__(self, chapter_id: str, translation_run_id: str, translation_segment_ids: tuple[str, ...]) -> None:
