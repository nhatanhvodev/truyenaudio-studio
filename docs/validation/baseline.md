...
| TDD GREEN: storage/API focused suite | PASS | `D:\truyenaudio-studio\.venv\Scripts\python.exe -m pytest backend/tests/storage/test_backup_restore.py backend/tests/api/test_storage_api.py -q` | Exit 0; `8 passed` in 4.19s. |
| Backend baseline | PASS | `D:\truyenaudio-studio\.venv\Scripts\python.exe -m pytest backend/tests -q` | Exit 0; `382 passed, 1 warning` in 79.04s. Warning: SQLAlchemy cannot topologically sort the existing FK cycle among `artifacts`, `chapters`, `exports`, `projects`, `source_revisions`, `translation_runs`, `voice_plans`, and `voice_presets`. |
...
`BackupService.restore_to_data_root()` requires the API and worker startup locks, a new non-existent target data root, and a different source root. It stages `studio.sqlite3` plus the artifact snapshot, validates the target database and pointers, then publishes the staged root. The fixture proves source data remains unchanged.
