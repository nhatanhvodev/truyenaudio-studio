...
| U07 Job UI + draft stream | ✅ | ✅ | ❌ | batch partial-failure + **route thật `POST /api/jobs/{id}/cancel|retry`** (wrapper `request_cancel`/`retry_failed`, mã lỗi có tên, chống gửi lại BILLING_UNKNOWN, idempotent); `JobsList` nối qua `jobActions`, còn `BatchQueue` **tự POST** hai route này và hiện lý do từ chối tại chỗ; **cancel nay quyết định trong chính câu UPDATE** — sửa race đọc-rồi-ghi do review độc lập phát hiện (job RUNNING bị ghi CANCELED ⇒ handler vẫn gọi provider và worker chết tiến trình; chiều ngược lại tạo zombie `CANCEL_REQUESTED` không lease), kèm **2 test tái hiện tất định đã xác nhận FAIL trên bản cũ**; còn **E2E SSE trên browser NOT_RUN** — xem `progress-upgrade-plan.md` |
