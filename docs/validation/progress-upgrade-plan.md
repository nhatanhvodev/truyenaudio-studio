...
| U02 App shell & Settings bảy nhóm | PARTIAL | Part 1: cây `/settings` đủ 7 nhóm C07 + nav aria-current + breadcrumb (4 test). Part 2: `GlobalNav` ba khu vực Thư viện/Công việc/Cài đặt gắn vào Shell (3 test). Nhóm Providers/Models/Translation đã có nội dung thật (U08/U09); còn: nested project routes để lấy project từ URL, loading/empty/error chuẩn hoá, nốt nhóm TTS/Storage/Appearance |
| U09 Quản lý style/glossary/nhân vật/memory | PARTIAL | P1 `StyleManager` 4 test; P2 `GlossaryManager` (locked/scope/forbidden, guard local không gọi API, báo affected+stale) 6 test; P3 `CharacterManager` (gender null→"chưa rõ", approve bắt buộc evidence, quan hệ có hướng + addressing theo chương) 6 test — đều gắn Settings→Translation. Còn: memory candidate review, nested project routes |
