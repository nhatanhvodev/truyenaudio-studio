...
| J04 Draft streaming resumable | PARTIAL | R1 engine (dedup/gap/bounded/cancel) 7 test; R2 snapshot endpoint 2 test; R3 SSE feed `/draft/stream` + `build_stream_events` tests; thêm nguồn delta thật: `QwenMtAdapter.stream_translate` (incremental_output + X-DashScope-SSE, dedupe cumulative, MALFORMED_STREAM/TRUNCATED) 7 test; còn: nối adapter delta vào SSE feed (R4) |
