...
| U04 Draft API và optimistic concurrency | PARTIAL | Round 1 (backend): bảng `workspace_drafts` (migration 0015, UNIQUE project/chapter/base revision) + service compare-and-swap (`DRAFT_REVISION_CONFLICT`, không ghi đè), validate segment thuộc base revision/size cap, draft không đụng approved; API `GET/PUT /api/chapters/{id}/draft` (200/409/400) — 6 test. Round 2 seam: `append_draft_delta` (append/dedupe/gap, `commit=False` cho caller có transaction) dùng bởi worker (J04 R4). Còn: wiring editor (restore/expectedRevision/conflict diff) |
