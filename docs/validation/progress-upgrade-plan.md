...
| U05 Tabs, dock và lưu layout | PARTIAL | Round 1: `workspaceLayout.ts` — reducer thuần (8 tab/pane; mở/đóng/reorder/split/dock/undock/reset), tab thứ 9 chỉ evict tab **sạch** (dirty → `TAB_LIMIT_DIRTY`, caller chọn lưu-rồi-đóng hoặc huỷ), đóng tab dirty bị chặn (`TAB_DIRTY`) trừ `force`, `undock` từ chối khi tràn (`UNDOCK_OVERFLOW`), guard chống leak cross-project (`PROJECT_MISMATCH`), serialize/parse layout có version + sanitize tab lạ/quá hạn (version lạ → layout mặc định + `migrated`) — 12 test. Còn: UI dock/tab + API lưu layout theo project, E2E keyboard dock/reopen |
