...
| J04 Draft streaming resumable | PARTIAL | Engine + snapshot + SSE feed (segmentReady) + adapter delta source (`stream_translate`) đã xong & test; phần còn lại (delta sống → feed job-scoped) dùng `workspace_drafts` của U04 (round 1 đã có backend) |
| U04 Draft API và optimistic concurrency | PARTIAL | Round 1 (backend): bảng `workspace_drafts` (migration 0015, UNIQUE project/chapter/base revision) + service compare-and-swap (`DRAFT_REVISION_CONFLICT`, không ghi đè), validate segment thuộc base revision/size cap, draft không đụng approved; API `GET/PUT /api/chapters/{id}/draft` (200/409/400) — 6 test. Còn: wiring editor (restore/expectedRevision/conflict diff) + worker ghi delta vào draft (mở J04 R4) |
