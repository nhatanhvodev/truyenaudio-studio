...
| U02 App shell & Settings bảy nhóm | PARTIAL | Part 1: cây `/settings` đủ 7 nhóm C07 + nav aria-current + breadcrumb (4 test). Part 2: `GlobalNav` ba khu vực Thư viện/Công việc/Cài đặt gắn vào Shell (3 test). Còn: nested project routes, loading/empty/error từng nhóm, nội dung thật nốt các nhóm |
| U08 Provider/model và quality settings | PARTIAL | Part 1: `features/modelCatalog/ModelCatalog` (filter + phân trang ≤100 + nhãn trung thực + stale/provenance) 6 test. Part 2: `features/providers/ProfileEditor` (profile list + credential masked, PUT một lần rồi xoá field, từ chối rỗng, validate/delete, tạo profile; localStorage rỗng) 5 test — gắn vào Settings→Providers. Còn: quality picker + quote/ceiling/consent từng stage |
