...
| U09 Quản lý style/glossary/nhân vật/memory | PARTIAL | P1 `StyleManager` (preset + revision/hash) 4 test + `ProjectScopedPanel`; P2 `GlossaryManager` (locked/scope/forbidden/description, guard local không gọi API khi sai, báo affected+stale sau lưu) 6 test — cả hai gắn ở Settings→Translation. Còn: character/relationship + memory candidate review, nested project routes |
