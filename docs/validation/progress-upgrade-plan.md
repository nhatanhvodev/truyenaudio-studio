...
| U09 Quản lý style/glossary/nhân vật/memory | PARTIAL | Part 1: `StyleManager` (preset + revision/hash, lưu style, trạng thái loading/empty/error) 4 test + `ProjectScopedPanel` (gate nhập Project ID, không persist) — gắn vào Settings→Translation; còn: glossary/character/memory managers + nested project routes |
