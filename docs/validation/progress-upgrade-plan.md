...
- Backend full suite (repo root): `.venv\Scripts\python.exe -m pytest backend/tests -q` → **647 passed**, 1 warning
...
| J04 Draft streaming resumable | PARTIAL | R1 engine (dedup/gap/bounded/cancel) 7 test; R2 snapshot endpoint 2 test; R3 SSE feed `/draft/stream` (Last-Event-ID, gap snapshot, terminal) + `build_stream_events` pure tests → 13 test draft; còn: live provider delta (adapters Phase 1 request/response) |
